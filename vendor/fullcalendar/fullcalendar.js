...
function EventManager(options) { // assumed to be a calendar
	...
	function eventContainsRange(event, start, end) {
		var Eventsart = event.start.clone().stripZone();
		var eventEnd = t.getEventEnd(event).stripZone();

		return start >= Eventsart && end <= eventEnd;
	}
	...
	function eventIntersectsRange(event, start, end) {
		...
		return start < eventEnd && end > Eventsart;
	}
...
$.extend(Grid.prototype, {
	...
	renderEvents: function(events) {
		var segs = this.EventsoSegs(events);
		var bgSegs = [];
		var fgSegs = [];
		var i, seg;
	...
	EventsoSegs: function(events, rangeToSegsFunc) {
		var eventRanges = this.EventsoRanges(events);
		var segs = [];
		var i;
			...
			if (eventGroup.length) {
				ranges.push.apply(
					...
					isInverseBgEvent(eventGroup[0]) ?
						_this.EventsoInverseRanges(eventGroup) :
						_this.EventsoNormalRanges(eventGroup)
				);
			}
		});
	...
	EventsoNormalRanges: function(events) {
		var calendar = this.view.calendar;
		var ranges = [];
		var i, event;
		var Eventsart, eventEnd;

		for (i = 0; i < events.length; i++) {
			event = events[i];

			// make copies and normalize by stripping timezone
			Eventsart = event.start.clone().stripZone();
			...
			ranges.push({
				...
				EventsartMS: +Eventsart,
				eventDurationMS: eventEnd - Eventsart
			});
		}
	...
	EventsoInverseRanges: function(events) {
		var view = this.view;
		var viewStart = view.start.clone().stripZone(); // normalize timezone
		var viewEnd = view.end.clone().stripZone(); // normalize timezone
		var normalRanges = this.EventsoNormalRanges(events); // will give us normalized dates we can use w/o copies
		var inverseRanges = [];
	...
	eventRangeToSegs: function(eventRange, rangeToSegsFunc) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			seg.EventsartMS = eventRange.EventsartMS;
...
function compareNormalRanges(range1, range2) {
	return range1.EventsartMS - range2.EventsartMS; // earlier ranges go first
}


// A cmp function for determining which segments should take visual priority
// DOES NOT WORK ON INVERTED BACKGROUND EVENTS because they have no EventsartMS/eventDurationMS
function compareSegs(seg1, seg2) {
	return seg1.EventsartMS - seg2.EventsartMS || // earlier events go first
...
$.extend(DayGrid.prototype, {
	...
	renderHelper: function(event, sourceSeg) {
		var helperNodes = [];
		var segs = this.EventsoSegs([ event ]);
		var rowStructs;
...
$.extend(TimeGrid.prototype, {
	...
	renderBusinessHours: function() {
		...
		this.businessHourSegs = this.renderFill('businessHours', this.EventsoSegs(events), 'bgevent');
	},
...
View.prototype = {
	...
	documentDragStart: function(ev, ui) {
		...
		var Eventsart = null; // a null value signals an unsuccessful drag
		var eventEnd = null;
			...
			if ($.isFunction(accept) ? accept.call(el[0], el) : el.is(accept)) {
				...
				dragListener = new DragListener(this.coordMap, {
					cellOver: function(cell, cellDate) {
						Eventsart = cellDate;
						eventEnd = meta.duration ? Eventsart.clone().add(meta.duration) : null;
						visibleEnd = eventEnd || calendar.getDefaultEventEnd(!Eventsart.hasTime(), Eventsart);

						// keep the start/end up to date when dragging
						if (eventProps) {
							$.extend(eventProps, { start: Eventsart, end: eventEnd });
						}

						if (calendar.isExternalDragAllowedInRange(Eventsart, visibleEnd, eventProps)) {
							_this.renderDrag(Eventsart, visibleEnd);
						}
						else {
							Eventsart = null; // signal unsuccessful
							disableCursor();
						}
					},
					cellOut: function() {
				...
				$(document).one('dragstop', function(ev, ui) {
					...
					if (Eventsart) { // element was dropped on a valid date/time cell

						// if dropped on an all-day cell, and element's metadata specified a time, set it
						if (meta.startTime && !Eventsart.hasTime()) {
							Eventsart.time(meta.startTime);
						}

						// trigger 'drop' regardless of whether element represents an event
						_this.trigger('drop', el[0], Eventsart, ev, ui);
